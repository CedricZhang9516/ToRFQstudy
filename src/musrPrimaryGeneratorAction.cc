...
void musrPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (takeMuonsFromTurtleFile) {
	...
	fgets(line,500,fTurtleFile);
      ...
      sscanf(&line[0],"%g %g %g %g %g %g %lf %d %d %lf",&xTmp,&xAngleTmp,&yTmp,&yAngleTmp,&pTmp,&zTmp,&t0,&Ztmp,&Atmp,&MUONIDtmp);
  ...
  G4cout <<"ptime otani"<< ParticleTime << G4endl;
  ...
  if (muonDecayTimeMax>0.) {
    ...
    G4cout << "decaytime otani " << decaytime << G4endl;
    decaytime = 2e6*ns;
